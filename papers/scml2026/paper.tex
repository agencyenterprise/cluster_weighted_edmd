...
\paragraph{Per-cluster Koopman operator.} For each cluster $k$ with center $c_k$, define the recentered monomial lift $\Phi: \mathbb{R}^d \to \mathbb{R}^{M_q}$ of total degree $\leq q$, where $M_q = \binom{d+q}{q}$. We use the bias-first EDMD convention with the constant monomial in entry $1$ followed by the $d$ linear monomials, $\Phi(z) = [1,\, z_1,\, \ldots,\, z_d,\, z_1^2,\, z_1 z_2,\, \ldots]^\top$, so that the linear monomials $z_1, \ldots, z_d$ occupy entries $2, \ldots, d+1$. The projection $P_d \in \mathbb{R}^{d \times M_q}$ defined by $P_d = [0_{d \times 1} \mid I_d \mid 0_{d \times (M_q - d - 1)}]$ then satisfies $P_d \Phi(z) = z$. With $K_k \in \mathbb{R}^{M_q \times M_q}$ the per-cluster Koopman matrix, the per-cluster prediction is
\[
