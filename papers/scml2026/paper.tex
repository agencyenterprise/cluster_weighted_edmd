...
\textbf{Experimental setup.} Three classical systems: Lorenz ($d{=}3$, polynomial-RHS deg-3), damped pendulum ($d{=}2$, non-polynomial $\sin\theta$), double-well Duffing ($d{=}2$, polynomial-RHS deg-3). Each: 12 configurations sweeping five sampling distributions, data size, domain, integrator step, and fit budget; 10 fixed seeds per configuration (full setup in Appendix~C; code and corpus in Appendix~F). We test \emph{matched-degree} CW-EDMD-$q$ against EDMD-$q$ via paired $t$-tests, holding the lift basis fixed to isolate the partitioning contribution; a \emph{cross-config win} is \textit{p}\,$<\,0.05$ with lower CW-EDMD mean.

% =====================================================================
% §2 Results
% =====================================================================
\begin{table}[H]
\caption{Headline matched-degree summary: paired-test wins of CW-EDMD vs.\ EDMD over the 12 configurations per system, and median one-step ratio EDMD / CW-EDMD ($>1$: CW-EDMD lower error).}
\label{tab:headline}
\centering\footnotesize
\begin{tabular}{lcr}
\toprule
System ($q$, $K$) & wins vs.\ EDMD & error ratio \\
\midrule
Pendulum (4, 8) & 12/12 & $\sim$2900$\times$ \\
Duffing  (3, 8) & 12/12 & $\sim$2.7$\times$ \\
Lorenz   (3, 12) & 10/12 & $\sim$35$\times$ \\
\bottomrule
\end{tabular}
\end{table}

\section{Results}
CW-EDMD outperforms EDMD at the matched polynomial lift on all three systems (Table~\ref{tab:headline}; per-system Pareto frontiers in Figures~\ref{fig:pareto-duffing}--\ref{fig:pareto-lorenz} and full tables in Appendix~D). On Duffing the matched comparison spans three polynomial degrees ($q\in\{3,4,5\}$, all with $12/12$ wins, paired $t$-test \textit{p}\,$<\,10^{-4}$ throughout); the two non-wins on Lorenz are characterized by the capacity-vs-data condition of Appendix~B. A direct within-EDMD ablation across all three systems isolates the residual-aware mechanism: CW-EDMD wins $271/324$ paired comparisons ($84\%$, Appendix~E).
...
EDMD $q{=}3$ (matches RHS deg)  & 100  & $6.6\!\times\!10^{-5}$ & $7.0\!\times\!10^{-3}$ \\
...
\begin{table}[H]
\caption{Lorenz attractor: median prediction error (dimensionless state-space units) across 12 configurations and 10 seeds. Top block: EDMD at two polynomial lift degrees. Middle block: CW-EDMD at lift degree $q{=}2$ (mismatched). Bottom block: CW-EDMD at the matched lift degree $q{=}3$.}
\label{tab:lorenz}
\centering\small
\begin{tabular}{lrrr}
\toprule
...
EDMD $q{=}3$                    & 400  & $2.2\!\times\!10^{-4}$ & $4.5\!\times\!10^{-2}$ \\
\midrule
