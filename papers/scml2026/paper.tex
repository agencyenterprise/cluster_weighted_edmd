...
\author{%
	Lorenzo Tomaz\thanks{Corresponding author.}, Judd Rosenblatt, Flavio Kicis, Thomas B. Jones, Diogo de Lucena \\
	AE Studio \\
...
pdfauthor={Lorenzo Tomaz, Judd Rosenblatt, Flavio Kicis, Thomas B. Jones, Diogo de Lucena},
...
\]
where $(\cdot)^{\dagger}$ denotes the Moore--Penrose pseudoinverse. We solve this in practice via SVD-based weighted least squares, which handles rank-deficient lifted Grams through SVD truncation rather than Tikhonov regularisation; no ridge term is added. The right-pseudoinverse ordering follows from $K$ acting on $\tilde{X}_k$ from the \emph{left} in the residual; if instead the model were $K \cdot \tilde{Y}_k = \tilde{X}_k$, the inverse would land on $\tilde{Y}_k$ and the order would flip. Finally $\sigma_k^2$ is updated from the responsibility-weighted residual variance.
...
\paragraph{Rollout.} At inference, given current state $x_t$, the rollout cluster index is $k^*(x_t) = \arg\max_k\, \pi_k\, \mathcal{N}(x_t; c_k, \Sigma_k)$ (geometry-only at test time, since no $x_{t+1}$ is available), and the next state is $x_{t+1} = c_{k^*} + P_d\, K_{k^*}\, \Phi(x_t - c_{k^*})$. Multi-step rollouts iterate this map. \emph{No derivatives, no Euler step.}
