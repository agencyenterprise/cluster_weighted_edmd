...
\paragraph{What is $x_i$?} In our setting, $x_i\in\R^d$ is a \textbf{state vector} of the dynamical system---a single point in phase space. For the Lorenz attractor, $x_i=(x,y,z)\in\R^3$ is a 3D position on the attractor. For the damped pendulum, $x_i=(\theta,\dot{\theta})\in\R^2$ is an angle and angular velocity. We have $P$ such state vectors, sampled from a trajectory or from the phase space directly.
...
\end{enumerate}
Step 2 says: given that state $x_i$ was assigned to cluster $k$ (i.e., $z_i=k$), its location in phase space is drawn from a Gaussian centered at $\mu_k$ with covariance $\Sv_k$. The term $p(x_i\mid z_i=k,\theta)=\mathcal{N}(x_i;\mu_k,\Sv_k)$ answers the question: \emph{``given that this state belongs to cluster $k$, how likely is it to be at this particular location in phase space?''} States close to the cluster center $\mu_k$ get high probability; states far away get low probability.
...
\begin{keyidea}
\textbf{Connection to Cluster-Weighted Models.} The model we are about to define is an instance of the \textbf{Cluster-Weighted Model} (CWM), a framework introduced by Gershenfeld (1997) and formalized statistically by Ingrassia, Minotti, and Vittadini (2012). A CWM models the joint density $p(x,y)=\sum_k\pi_k\,p(x|k)\,p(y|x,k)$---combining a marginal density on inputs $p(x|k)$ with a conditional regression on outputs $p(y|x,k)$. In the Linear Gaussian CWM, both are Gaussian with an affine conditional mean.

Our model is a CWM where $x$ is a state vector and $y=f(x)$ is the vector field. The standard data-driven affine local model (our Variant~B) is exactly a Linear Gaussian CWM. Our contributions are: (i)~a physics-constrained variant where the local model is the analytic Taylor expansion (Variant~A, no precedent in CWM literature), (ii)~a Koopman-theoretic variant with lifted-space generators (Variant~C), and (iii)~application to dynamical-systems phase-space partitioning, an entirely new domain for CWM.

Sections 1--5 presented standard Bayesian GMM machinery. From this point on, everything is specific to the CWM formulation for dynamical systems.
\end{keyidea}
...
Line~\eqref{eq:prox-model} is the proximity model (standard GMM). Line~\eqref{eq:resid-model} is the residual model: it says that the observed vector field $f_{\mathrm{obs}}(x_i)$ is drawn from a Gaussian centered at the local model's prediction $L_k(x_i)$, with isotropic noise $\sigma^2 I$. Together, lines~\eqref{eq:prox-model}--\eqref{eq:resid-model} define a \textbf{Cluster-Weighted Model} (CWM)---the joint-density factorization $p(x,y)=\sum_k\pi_k\,p(x|k)\,p(y|x,k)$ introduced by Gershenfeld (1997) and formalized by Ingrassia et al.\ (2012).
...
\end{align}
The first two terms are identical to standard GMM. The third term---the log-residual---is what distinguishes the CWM from a standard GMM: it couples the cluster assignments to the local regression quality. This factorization is due to Gershenfeld (1997) and Ingrassia et al.\ (2012). Substituting into $Q$:
\begin{equation}
...
The $Q_\pi$ and $Q_{\text{prox}}$ terms are identical to standard GMM. The $Q_{\text{res}}$ term (the CWM regression component) is:
\begin{equation}
...
\begin{enumerate}[leftmargin=*,itemsep=0pt]
  ...
  \item $(R_k/\sigma^2)\Jv_k^\top\Jv_k$: pull toward the point that minimizes the residual (the CWM regression term).
\end{enumerate}
...
For a single cluster $k$ with points $\{x_i:z_i=k\}$, the \emph{true} marginal likelihood would integrate out $(c_k,\Sv_k)$ from the \emph{full} likelihood (proximity $\times$ residual):
\begin{equation}
  p(X_k,F_k) = \int p(X_k\mid c_k,\Sv_k)\;p(F_k\mid X_k,c_k,\sigma^2)\;p(c_k,\Sv_k)\;dc_k\,d\Sv_k.
  \label{eq:true-marginal}
\end{equation}
However, this integral is \textbf{not available in closed form}. The reason: the residual factor $p(F_k\mid X_k,c_k,\sigma^2)$ depends on $c_k$ through the local model $L_k(c_k,\cdot)$---via $f(c_k)$ and $\Jv(c_k)$ in the Taylor variant, or via the LS-fit operator conditioned on $c_k$. This nonlinear dependence on $c_k$ breaks the NIW conjugacy. The proximity factor alone is conjugate to the NIW prior; the product of both factors is not.

\paragraph{What we compute instead: a plug-in approximation.} We separate~\eqref{eq:true-marginal} into two parts:

\textbf{Part 1: exact NIW marginal for the proximity factor.} The integral
\begin{equation}
  p(X_k) = \int p(X_k\mid c_k,\Sv_k)\,p(c_k,\Sv_k)\,dc_k\,d\Sv_k
\end{equation}
\emph{is} closed-form because the Gaussian likelihood $p(X_k\mid c_k,\Sv_k)$ is conjugate to the NIW prior:
\begin{align}
...
\textbf{Part 2: residual penalty at the converged point estimate $\hat{c}_k$.} We evaluate the residual factor at the EM-converged center rather than integrating it out:
\begin{equation}
  \log\widetilde{p}(X_k,F_k) = \log p(X_k) - \frac{1}{2\sigma^2}\sum_{i:z_i=k}\|\varepsilon_k(x_i)\|^2\Biggr|_{c_k=\hat{c}_k}.
\end{equation}

\begin{remark}[Plug-in approximation]
This is \textbf{not an exact marginal likelihood}---it is a hybrid of an exact integral (proximity part) and a plug-in evaluation (residual part). The approximation is adequate when the posterior on $c_k$ is concentrated (large $R_k$, many points in the cluster), because the integrand in~\eqref{eq:true-marginal} is then sharply peaked around $\hat{c}_k$ and the plug-in value closely approximates the integral. In the limit $\sigma^2\to\infty$ (no residual term), it reduces to the exact NIW marginal. In the limit of very small clusters with few points, the approximation degrades and BIC may be a more reliable model-selection criterion.
\end{remark}

The total approximate log marginal is:
\begin{equation}
  \log\widetilde{p}(X,F\mid N) = \underbrace{\log\frac{\Gamma(N\alpha_0)}{\Gamma(P+N\alpha_0)}\prod_k\frac{\Gamma(R_k+\alpha_0)}{\Gamma(\alpha_0)}}_{\text{Dirichlet-Categorical marginal}} + \sum_k\log\widetilde{p}(X_k,F_k).
\end{equation}
The model with the highest $\log\widetilde{p}(X,F\mid N)$ is preferred.
